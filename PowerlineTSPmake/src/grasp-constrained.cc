...
CMKGREED::CMKGREED(crl::CConfig &config) : Base(config, "TRIAL"),
  ...
  base_coords(Coords(config.get<double>("base_lat"),config.get<double>("base_lon"))),
        ...
        for (int i = 0; i < averagePathSize; i++) {
          for (int j = 0; j < m; j++) {
            for(int k = 0; k < currentSolution[j].size();k++){
                ...
                TargetSetPtrVector sourcePath = TargetSetPtrVector(currentSolution.at(j));
                TargetSetPtrVector targetPath = TargetSetPtrVector(currentSolution.back());
                ...
                possibleShifts.insert(TargetShift(shiftCost,j,k,m,l+1,false));
                }
                }
                }
...
void CMKGREED::drawPath(void)
{
  ...
  for(int i = 0; i < m; i++)
  {
    for(int j = 0; j < finalSolution.at(i).size(); j++)
    {
      ...
      //path.emplace_back(finalSolution.at(i).at(j));
    }
  }
...
TargetSetPtrVectorVector CMKGREED::getTSSolution(TargetSetPtrVectorVector& prevSol, int nodes)
{
  int totalScore;
  int random;
  ...
  TargetSetPtrVectorVector bestNeigborhoodSolution = prevSol;
...
void CMKGREED::getG1Solution(TargetSetPtrVectorVector& prevSol) // random shift intra-inter route
{
  ...
  if (std::rand() % 2 < 1){ // shift intra route
    indexA2 = indexA1;
    do{
      indexC2 = std::rand() % prevSol.at(indexA1).size();
    ...
    indexC2 = std::rand() % (prevSol.at(indexA2).size());
...
void CMKGREED::getG2Solution(TargetSetPtrVectorVector& prevSol) // best shift intra-inter route based on exhaustive search
{
  //DEBUG("G2 invoked");
  int routes = prevSol.size();
  int indexA1 = std::rand() % routes;
...
void CMKGREED::getG3Solution(TargetSetPtrVectorVector& prevSol) // best swapt intra-inter route based on exhaustive search
{
  //DEBUG("G3 invoked");
  int routes = prevSol.size();
  int indexA1 = std::rand() % routes;
...
double solutionLength = std::min(std::min(std::min(solutionLength1, solutionLength2),solutionLength3),solutionLength4);
...
double CMKGREED::get_path_cost(const TargetSetPtrVector &targetsets)
{
   ...
   if(targetsets.size() > 2){
      ...
      for (unsigned long i = 1; i < targetsets.size(); i++) {
          pathlen = targetsets[i-1]->distance_to(*targetsets[i]);
          if(pathlen > maxCost){
            pathlen += (pathlen-maxCost)*1000;
        }
      }
      len += pathlen;
    }
