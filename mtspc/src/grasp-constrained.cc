...
void CMKGREED::iterate(int iter) 
{
      ...
      if(targets.at(i)->visited == false){
        ...
        for(int j = 0; j < m; j++){   
          for(int k = 1; k <= currentSolution.at(j).size(); k++){
            ...
            cost = get_path_cost(costCoords,maxCost,pylonCost); 
  ...
  while(!validSolution){
    currentSolution = getTSSolution(currentSolution, step);
    DEBUG("Final cost: "<<get_solution_cost(currentSolution,maxCost,pylonCost));
    validSolution = true;

    for(auto somepath : currentSolution){
      if(validSolution && get_path_cost(somepath,maxCost,pylonCost) > maxCost){
        validSolution = false;
        ...
        for (int i = 0; i < averagePathSize; i++) {
          for (int j = 0; j < m; j++) {
            for(int k = 1; k < currentSolution.at(j).size();k++){
                ...
                double shiftCost = get_path_cost(sourcePath,maxCost,pylonCost)+get_path_cost(targetPath,maxCost,pylonCost);
...
CoordsVectorVector CMKGREED::getTSSolution(CoordsVectorVector& prevSol, int nodes)
{
  ...
  double bestSolutionLength = get_solution_cost(prevSol,maxCost,pylonCost);
  ...
  for(int iteration = 0; iteration < tabuIterationsDynamic; iteration++)
  {
    ...
    for(int j = 0; j < nodes*m; j++)
    {
      ...
      tabuSolutionLength = get_solution_cost(tabuSolution,maxCost,pylonCost);
  ...
  for(int i = 0; i < m; i++)
  {
    for(int j = 1; j <= prevSol.at(i).size(); j++)
    {
      if(!((i == indexA1) && (j == indexC1)))
      {
        ...
        solutionLength = get_solution_cost(intermediateSolution,maxCost,pylonCost);
